...
- **デザイン規範 (確定)**: 等幅フォント / 高密度・隙間なし (zebra+極薄罫線) / カード・ピル・ボタン箱・下線リンク風は禁止 / 操作は [ラベル] ブラケット表記 / ラベルは入力の左 (同一行) / 入力欄は明確な箱 (raised背景+全周ボーダー+角丸6px、ピル不可) / 色数最小 (accent+dangerのみ)、区別は絵文字で / 面の分離 (bg と raised) を明確に
